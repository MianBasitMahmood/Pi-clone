...
public class ViewDragHelper {
    ...
    public static ViewDragHelper create(ViewGroup viewGroup, float f, Callback callback) {
        ...
        create.mTouchSlop = (int) (((float) create.mTouchSlop) * (TextTrackStyle.DEFAULT_FONT_SCALE / f));
        return create;
    }

    private ViewDragHelper(Context context, ViewGroup viewGroup, Callback callback) {
        this.mActivePointerId = INVALID_POINTER;
        this.mSetIdleRunnable = new C00772();
        if (viewGroup == null) {
    ...
    private int computeAxisDuration(int i, int i2, int i3) {
        ...
        float distanceInfluenceForSnapDuration = (distanceInfluenceForSnapDuration(Math.min(TextTrackStyle.DEFAULT_FONT_SCALE, ((float) Math.abs(i)) / ((float) width))) * ((float) i4)) + ((float) i4);
        i4 = Math.abs(i2);
        if (i4 > 0) {
